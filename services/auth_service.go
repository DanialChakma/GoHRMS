...
func (s *AuthService) Login(ctx context.Context, username, password string) (access, refresh string, err error) {
	...
	role := models.RoleFromID(user.RoleID).String()
	access, _ = s.tokens.GenerateJWT(user.Username, role, user.TokenVersion)
	jti := uuid.New().String()
	refresh, _ = s.tokens.GenerateRefreshJWT(user.Username, role, jti)
...
func (s *AuthService) Refresh(ctx context.Context, oldRefreshToken string) (string, string, error) {
	...
	accessToken, err := s.tokens.GenerateJWT(user.Username, role, user.TokenVersion)
	if err != nil {
		return "", "", errors.New("failed to generate access token")
	}

	newJTI := uuid.New().String()
	refreshToken, err := s.tokens.GenerateRefreshJWT(user.Username, role, newJTI)
	if err != nil {
